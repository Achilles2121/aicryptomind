...
import { detectOrderBlocks } from "../server/liquidity/orderBlocks.js";
import { detectFairValueGaps } from "../server/liquidity/fairValueGaps.js";
import { detectImbalanceZones } from "../server/liquidity/imbalanceZones.js";
import { buildLiquidityHeatmap } from "../server/liquidity/liquidityHeatmap.js";
import { detectWhaleMoves } from "../server/liquidity/whaleTracker.js";
...
export default async function handler(req: Req, res: Res) {
  try {
    ...
    const limitParam = typeof req.query?.limit === "string" ? Number(req.query.limit) : 180;
    const limit = Number.isFinite(limitParam) ? Math.max(60, Math.min(400, limitParam)) : 180;
    ...
    const key = cacheKey("liquidity", symbol, interval, limit);
    ...
    const imbalances = detectImbalanceZones(candles);
    const heatmap = buildLiquidityHeatmap(candles);
    const whales = detectWhaleMoves(candles);
    ...
    return send(res, 200, {
      ...
      symbol: "BTCUSDT",
      interval: "1h",
      candles: generateFakeSeries(120),
