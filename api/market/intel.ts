...
import { computeSentiment } from "../../server/market/sentiment.js";
import { detectAnomalies } from "../../server/market/anomalies.js";
import { assessRegime } from "../../server/market/volatilityRegime.js";
import { scanLiquidations } from "../../server/market/liquidationScanner.js";
...
async function fetchKraken(symbol: string, interval: string, limit: number): Promise<Candle[]> {
  ...
  const url = `https://api.kraken.com/0/public/OHLC?pair=${mapped}&interval=${interval === "1h" ? 60 : interval === "15m" ? 15 : 240}`;
  const data = await safeFetchJson<{ result: Record<string, (number | string)[]> }>(
    url,
    undefined,
    { timeoutMs: 3200, attempts: 3 }
  );
...
async function resolveOHLC(symbol: string, interval: string, limit: number): Promise<Candle[]> {
  const providers = [
...
export default async function handler(req: Req, res: Res) {
  try {
    ...
    const limit = Number.isFinite(limitParam) ? Math.max(60, Math.min(400, limitParam)) : 160;

    const rateKey = req.headers?.["x-forwarded-for"] ?? "anon";
    if (isRateLimited(`market-intel:${rateKey}`)) {
      return send(res, 429, { ok: false, error: "rate_limited" });
    }

    const key = cacheKey("market-intel", symbol, interval, limit);
    ...
    // Synthetic/heuristic scores
    const newsScore = Math.sin(Date.now() / 60000) * 10;
    const socialScore = Math.cos(Date.now() / 50000) * 15;

    const { fearGreed, sentiment } = computeSentiment({
      candles: candles as unknown as any[],
      newsScore,
      socialScore,
    });
    const anomalies = detectAnomalies(candles as unknown as any[]) as {
      anomalyScore: number;
      volumeSpikes: any[];
      spreadAnomalies: any[];
    };
    const regime = assessRegime(candles as unknown as any[]) as {
      regimeScore: number;
      regimeLabel: string;
    };
    const liquidations = scanLiquidations(candles as unknown as any[]) as any[];

    const regimeScore = regime.regimeScore;
    const anomalyScore = anomalies.anomalyScore;
    const marketStatus =
      regime.regimeLabel === "high-vol"
        ? "watch-volatility"
        : fearGreed > 65
        ? "risk-on"
        : fearGreed < 35
    ...
    const payload = {
      ...
      symbol,
      interval,
      fearGreedScore: Number(fearGreed.toFixed(1)),
      sentimentScore: Number(sentiment.toFixed(1)),
      regimeScore: Number(regimeScore.toFixed(2)),
      anomalyScore: Number(anomalyScore.toFixed(1)),
      liquidationClusters: liquidations,
      marketStatus,
      components: {
        volumeSpikes: anomalies.volumeSpikes,
        spreadAnomalies: anomalies.spreadAnomalies,
