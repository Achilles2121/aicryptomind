...
## Aktuelle Änderungen (Dez 2025)

- **Modul 9 – Deployment-Check**: Neues `vercel.json` mit Vite-Build, statischem Output, Edge-Rewrites und Cache-Headern; frischer `npm run build` Erfolg (vgl. Deploy-Log unten); neue `docs/deploy-guide.md` beschreibt lokale Tests, Vercel-Workflow, Routing und API-Latenz-Checkliste.
- **Modul 10 – Investor-README**: `docs/investor-readme.md` fasst Produktpitch, Architektur, Datenquellen, Tier/Trial-System, Indikatoren-Stack, Security-Notizen und Mobile-Ready-Story für Investoren/Stakeholder.
- **Chart-Loader-Integration (WIP)**: `src/App.jsx` nutzt jetzt `loadChart`/`buildFallbackChart`. Echtzeit-Feed hängt noch am alten WebSocket/Polling-Aufbau; solange `fetchWithCache` (TTL 5 min) aktiv ist, aktualisiert sich der Chart nur nach Cache-Expiry. WS-Rehydration + kürzeres TTL stehen noch aus (siehe „Bekannte Einschränkungen“).
...
Senior TypeScript/React/Vite engineer + HF crypto trading architect. Ziel: Crypto Risk Manager V3 auf V3+ (institutionell) heben, Lookahead eliminieren, Risikokennzahlen implementieren.
...
- Lösung: Dedizierte HTF-Fetches (z. B. in `src/services/marketDataLive.ts`) von Kraken OHLC (`interval=240/1440`) oder CoinAPI; kein Resampling.
- Health Keys ergänzen: `MARKET_HTF_PRIMARY`, `MARKET_HTF_FALLBACK`.
...
- Wrapper für Research-Tab-Ausgabe via ChainGPT API (oder OSS LLM + RAG spezialisiert auf Krypto).
...
- Ziel: Crypto Risk Manager von V3 auf V3+ (institutionell); Lookahead entfernen, Risikometriken absichern; UI-Layout unverändert lassen.

### Architektur & Health-Regeln

- Stack: Vite + React/TS; zentrale Fetch-Schicht bleibt `src/lib/safeFetch.ts` (nicht grundlegend ändern).
